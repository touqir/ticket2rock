...
* <tt>mvn integration-test</tt> startet die Integrationstests (Vorher muss der JBoss gestartet werden)
...
1. <tt>mvn package</tt>
2. Unter Linux/Mac <tt>export JBOSS_HOME=/path/to/jboss-6.0.0.Final</tt> bzw. unter Windows setzen Sie die Variable in der Systemsteuerung.
...
Dafür muss in der entsprechenden Konfigurationsdatei '$JBoss_HOME/server/default/deploy/hornetq/hornetq-jms.xml folgende Queue vermerkt werden:
...
Die Integrationstests laufen teilweise auf Fehler, wenn die Anwendung gleichzeitig deployt ist. Damit alle Tests erfolgreich sind,
...
In der aktuellen Ruby Version (1.9.1) führt das zu einem Fehler.
In der Version 1.8 funktioniert es aber noch.
